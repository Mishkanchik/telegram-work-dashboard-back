...
function createUser($telegram_id, $username, $full_name, $referred_by_code = null) {
    $db = getDB();

    // Генерація реферального коду
    $referral_code = 'ref_' . $telegram_id . '_' . substr(md5(uniqid()), 0, 6);

    // Пошук реферера
    $referred_by_id = null;
    if ($referred_by_code) {
        $stmt = $db->prepare("SELECT id FROM users WHERE referral_code = :code");
        $stmt->bindValue(':code', $referred_by_code, SQLITE3_TEXT);
        $result = $stmt->execute();
    ...
    $stmt->bindValue(':rcode', $referral_code, SQLITE3_TEXT);
    $stmt->bindValue(':rbid', $referred_by_id, $referred_by_id ? SQLITE3_INTEGER : SQLITE3_NULL);
    $stmt->execute();

    logAction($telegram_id, 'register', "Referred by: " . ($referred_by_code ?? 'none'));
...
function startSession($user_id, $shift_type) {
    ...
    // Перевірка часу (не можна після 23:00)
    $hour = (int)date('H');
    if ($hour >= 23) {
        return 'time_limit';
    }
...
function autoCloseExpiredSessions() {
    ...
    while ($session = $result->fetchArray(SQLITE3_ASSOC)) {
        ...
        // Закрити, якщо триває більше 12 годин або минула 23:59
...
function getMainMenuKeyboard($user) {
    ...
    $keyboard = [
        'inline_keyboard' => [
            ...
            [
                ['text' => '📊 Статистика', 'web_app' => ['url' => WEBAPP_URL . '/stats.php?user_id=' . $user['telegram_id']]],
